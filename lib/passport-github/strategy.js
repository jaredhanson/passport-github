...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  self._oauth2._request("GET", self._userProfileURL, {
    ...
    "Accept": "application/vnd.github.v3.full+json",
        ...
        var email, verifiedPrimaryEmail, unverifiedPrimaryEmail;
        var unverifiedEmails = [];
        var verifiedEmails = [];
        (emails || []).forEach(function (e) {
          if (e.verified) {
            verifiedEmails.push(e.email);
            if (e.primary)
            {
              //Assuming there is only one primary email
              verifiedPrimaryEmail = e.email;
              //Ensures that the email chosen is the verified primary if it exists
              email = e;
            }
            //prefer a verified email over unverified
          ...
          {
            unverifiedEmails.push(e.email);
            if (e.primary)
            {
              //Assuming there is only one primary email
              unverifiedPrimaryEmail = e.email;
        ...
        profile.emails = [{ value: email.email }];
        profile.allEmails = emails;
        profile.verifiedPrimaryEmail = verifiedPrimaryEmail;
        profile.unverifiedPrimaryEmail = unverifiedPrimaryEmail;
        profile.verifiedEmails = verifiedEmails;
        profile.unverifiedEmails = unverifiedEmails;
