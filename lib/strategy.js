...
function Strategy(options, verify) {
  ...
  // Determines whether or not we should make an additional call for private
  // emails before returning the profile.
  this._supplementEmail = false;
  if (options.scope && 
      (options.scope.indexOf('user') >= 0 || options.scope.indexOf('user:email') >= 0)){
    this._supplementEmail = true;
  }
}
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    self._oauth2.get(self._userEmailURL, accessToken, function (err, body, res) {
      ...
      if (!profile._json.email){
        ...
        // Add the user's public email that they claim.
        profile.emails = [{email: profile._json.email, public:true, 
          verified: false}];
      }

      try {
        emailJson = JSON.parse(body);
        if (emailJson){
          emailJson.forEach(function(email){
            email.public = false;
