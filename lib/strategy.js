 ...
 *   — `email`         boolean to indicate email addresses are required
 *
 * Examples:
 *
...
function Strategy(options, verify) {
  ...
  this._email = options.email || false;
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    if (self._email) {
      self._oauth2._request("GET", self._userProfileURL + "/emails", {"Accept": "application/vnd.github.v3.full+json"}, "", accessToken, function (err, body, res) {
        var emails;
        try {
          emails = JSON.parse(body);
        }
        catch (ex) {
          return done(new Error('Failed to parse user email addresses'));
        }
        ...
        (emails || []).forEach(function (email) {
          if (email.verified) {
            profile.emails.push({value: email.email})
          }
        });
        done(null, profile);
      });
